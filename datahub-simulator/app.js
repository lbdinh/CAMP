                ...
                h('div', { className: "flex-none min-h-24 border-b border-brand-medium/20 bg-gradient-to-r from-brand-dark/90 via-brand-navy/85 to-brand-panel/80 flex items-center justify-between px-8 z-30 backdrop-blur-md" },
                    h('div', { className: "flex items-center h-full gap-5" },
                        h('a', {
                            ...
                            h('img', { 
                                src: "../CA_Logo_Color-white.png", 
                                alt: "CalAmp", 
                                className: "h-14 w-auto shrink-0 opacity-100 transition-transform duration-300 hover:scale-105" 
                            })
                        ),
                        h('div', { className: "flex items-end h-full pt-4" },
                    ...
                    h('div', { className: `absolute left-3 top-3 bottom-3 ${isCollapsed ? 'w-12' : 'w-52'} transition-all duration-300 ease-in-out bg-brand-panel/95 backdrop-blur-md border border-brand-medium/20 rounded-xl flex flex-col z-40 overflow-hidden shadow-2xl` },
                        ...
                        h('div', { className: `flex-1 ${isCollapsed ? 'p-2 pt-6 items-center' : 'p-4 overflow-y-auto'} flex flex-col gap-3.5` },
                            h(ControlButton, {
                                ...
                                isActive: isPlaying,
                                activeClass: 'bg-brand-orange/20 border-brand-orange text-white hover:bg-brand-orange/30 shadow-[0_0_15px_rgba(255,150,90,0.25)]',
                    ...
                    h('div', { className: "relative w-full max-w-5xl h-full flex justify-center" },
                        ...
                        h('div', { className: "relative w-96 h-full z-20" },
                            ...
                            h(StageNode, {
                                ...
                                title: MODE_LABELS[mode].replace('AWS KINESIS', 'AWS KINESIS STREAM').replace('Google PUB/SUB', 'Google Pub/Sub Topic') + (mode === 'webhook' ? ' ENDPOINT' : mode === 'polling' ? ' SCRIPT' : ''),
